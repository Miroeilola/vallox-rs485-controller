...
// and vallox_protocol.h. `conf` mirrors protocol.md's confidence ratings:
// manufacturer = in Vallox's own document, implementations = agreed by the
// four independent implementations / reverse-engineered, assumed = reported
// by one source or inferred. protocol.md's register map is prefaced
// "Compiled from four implementations. The Read/Write column is what the
// sources claim, not what has been tested" — so every row below is
// implementations-class, even where the same register also appears in the
// manufacturer's numbered claims list. A register absent from this table is
// unknown and gets NO answer. Defaults describe a plausible idle machine in
// a Finnish autumn, not a real one.
...
static const vlx_machine_reg_t k_regs[] = {
    ...
    { VLX_REG_DEFROST_HYSTERESIS,  true, 0x09, VLX_CONF_IMPLEMENTATIONS }, // x/3 °C: 0x09 = 3 °C
};

// Temperature registers: the default is in degrees and converted at init.
static const temp_reg_def_t k_temp_regs[] = {
    { VLX_REG_TEMP_OUTDOOR,       false,  5, VLX_CONF_IMPLEMENTATIONS },
    ...
    // supply fan stop (frost protection) threshold, −6…+15 °C per protocol.md;
    // the former hard-coded −2 °C limit is now just this register's default.
    { VLX_REG_SUPPLY_FAN_STOP,    true,  -2, VLX_CONF_IMPLEMENTATIONS },
};
